...
class Session:
    ...
    def apply(self, note: str = "") -> ChangeReport:
        ...
        target = self.workspace / f"v{len(self.versions):03d}-edited.pptx"
        try:
    ...
    def rollback(self, to: int = 0) -> Version:
        ...
        if not 0 <= to < len(self.versions):
            raise SessionError(f"no version {to}; have 0..{len(self.versions) - 1}")
        self.versions = self.versions[: to + 1]
        self.changeset = None
